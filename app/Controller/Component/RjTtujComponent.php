...
class RjTtujComponent extends Component {
    ...
    function _callBeforeViewReportRecapBonBiru( $params ) {
        $allow_branch_id = Configure::read('__Site.config_allow_branch_id');
        $customers = $this->controller->Ttuj->Customer->getData('list', array(
            'fields' => array(
                'Customer.id', 'Customer.customer_name_code'
        ...
        $this->controller->set('sub_module_title', __('Laporan Rekap Penerimaan Bon Biru'));
        $this->controller->set('active_menu', 'report_recap_bon_biru');
        $this->controller->set(compact(
            'period_text', 'customers', 'cities'
        ));
    }
