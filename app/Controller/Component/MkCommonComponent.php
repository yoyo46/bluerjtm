...
class MkCommonComponent extends Component {
    ...
    */
    ...
    function processFilter ( $data ) {
        ...
        $nodocs = $this->filterEmptyField($data, 'Search', 'nodocs');
        $params = array();
        ...
        if( !empty($nodocs) ) {
            if( is_array($nodocs) ) {
                $nodocs = implode(',', $nodocs);
            }

           $params['nodocs'] = rawurlencode(urlencode($nodocs));
        }
    ...
    function _callRefineParams ( $data, $options = array() ) {
        ...
        $nodocs = $this->filterEmptyField($result, 'named', 'nodocs');
        ...
        if( !empty($nodocs) ) {
            if( !is_array($nodocs) ) {
                $nodocs = urldecode(rawurldecode($nodocs));
                $codeArr = explode(',', $nodocs);
            } else {
                $codeArr = $nodocs;
            }
            ...
            if( !empty($codeArr) && count($codeArr) > 1 ) {
                $result['named']['nodocs'] = $code;
            } else {
                $result['named']['nodocs'] = $code;
            }
            
            $codeTmp = array_combine($code, $code);
            $this->controller->request->data['Search']['nodocs'] = $code;
            $this->controller->request->data['Search']['nodocs_options'] = $codeTmp;
        }
    ...
    function _callDemoVersion () {
        if( in_array(FULL_BASE_URL, array( 'http://ww.erprjtm.com', 'http://ww.rjtm.com', 'http://erp.rjtm.co.id', 'http://e.rjtm.co.id', 'http://rjtm-jkt.com', 'http://dev.rjtm-jkt.com' )) ) {
            return true;
        } else {
            return false;
        }
