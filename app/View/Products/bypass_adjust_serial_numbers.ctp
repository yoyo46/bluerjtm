...
<div class="form-serial-number">
	<?php 
			if( $status == 'success' ) {
				...
				echo $this->Form->create('ProductAdjustmentDetailSerialNumber', array(
				...
				if( !empty($number) ) {
					...
					for ($i=0; $i < $number; $i++) { 
						$dataArr[] = $this->Html->tag('li', $this->Common->buildInputForm(sprintf('ProductAdjustmentDetailSerialNumber.serial_number.%s', $i), false, array(
							'fieldError' => sprintf('ProductAdjustmentDetailSerialNumber.%s.serial_number', $i),
						)), array(
							'class' => 'col-sm-4',
						));
					}
				...
				echo $this->Form->hidden('ProductAdjustment.session_id');
				echo $this->Form->end();
			}
	?>
</div>