...
<div class="table-responsive">
    <table class="table">
    	...
    	// 				'style' => 'width: 100px;'
					// ));
    	// 			$tdContent .= $this->Html->tag('td', $this->Common->combineDate($invoice['Invoice']['period_from'], $invoice['Invoice']['period_to'], 'M Y'));
    	// 			echo $this->Html->tag('td', $tdContent);
    		?>
	    </tbody>
	</table>
</div>
	...
	<table border="1" width="100%" style="margin-top: 20px;">
		<thead class="header-invoice-print">
			<tr>
				...
				<th class="text-center" rowspan="2"><?php echo __('No. Truk');?></th>
				<th class="text-center" rowspan="2"><?php echo __('No. SJ');?></th>
				<th class="text-center" rowspan="2"><?php echo __('Nomor');?></th>
				<th class="text-center" colspan="3"><?php echo __('Shipping list');?></th>
				<th class="text-center" rowspan="2"><?php echo __('Unit');?></th>
				<th class="text-center" rowspan="2"><?php echo __('Rate');?></th>
				<th class="text-center" rowspan="2"><?php echo __('Amount');?></th>
				<th class="text-center" rowspan="2"><?php echo __('Ket');?></th>
			</tr>
			<tr>
				<th class="text-center"><?php echo __('Nomor.');?></th>
			...
			<?php
					if(!empty($invoice['Revenue'])){
						...
						foreach ($invoice['Revenue'] as $key => $revenue) {
							...
							$unit = !empty($invoice['qty_unit'])?$invoice['qty_unit']:0;
							$price_unit = !empty($invoice['price_unit'])?$invoice['price_unit']:0;
			?>
			<tr>
				<?php 
						...
						if( !empty($revenue['RevenueDetail'][0]) ) {
							...
							echo $this->Html->tag('td', $revenueDetail['qty_unit']);
						...
						$totalPrice = $unit*$price_unit;

						if( !empty($totalPrice) ) {
							echo $this->Html->tag('td', $this->Number->format($totalPrice, Configure::read('__Site.config_currency_code'), array('places' => 0)), array(
								'style' => 'text-align: right;'
							));
						} else {
							echo $this->Html->tag('td', 0, array(
